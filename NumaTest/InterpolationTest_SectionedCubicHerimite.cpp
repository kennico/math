...
TEST_P(SectionedCubicHermiteTest, Quadratic) {
    ...
    Helper<SectionedCubicHermite>::Container container;
    std::for_each(d.begin(), d.end(), [&](auto n) {
        container.emplace_back(n, quadratic(n), quadraticdy(n));
    });

    auto intrpl = Helper<SectionedCubicHermite>().from(container);
...
//TEST(SectionedCubicHermiteTest, Sample) {
//
